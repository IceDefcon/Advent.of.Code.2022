...
int main(int argc, char const *argv[])
{
    ...
    while (getline (file_2, buffer)) 
    {
    	...
    	else if(folder_parameters[count].size == 0 && folder_parameters[count].amount > 0) folder_parameters[count].flag = true;
    ...
    for (int i = 0; i < count; ++i)
    {
    	...
    	{
		    cout
		    << " ("  << folder_parameters[i].flag
		    << ") (" << folder_parameters[i].amount
		    << ") (" << folder_parameters[i].count
		    << ") (" << folder_parameters[i].x
		    << ") (" << folder_parameters[i].size
		    << ","   << folder_parameters[i].name
		    << ") "  << endl;

	    	temp = temp + folder_parameters[i].size * current;
